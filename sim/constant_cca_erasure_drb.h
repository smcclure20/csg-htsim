...
#ifndef CONSTANTERASURECCADRB_H
#define CONSTANTERASURECCADRB_H
...
#include "ecn.h"
#include "uec_mp.h"
#include "datacenter/fat_tree_topology_drb.h"

#define timeInf 0

class ConstantErasureCcaSrcDRB;
class ConstantErasureCcaSinkDRB;
class ConstErasureBaseScheduler;

class ConstantErasureCcaPacerDRB : public EventSource {
public:
    ConstantErasureCcaPacerDRB(ConstantErasureCcaSrcDRB& src, EventList& eventlist, simtime_picosec interpacket_delay);
...
class ConstantErasureCcaSrcDRB : public EventSource, public PacketSink, public ConstScheduledSrc {
    friend class ConstantErasureCcaSinkDRB;
public:
    ConstantErasureCcaSrcDRB(EventList &eventlist, uint32_t addr, simtime_picosec pacing_delay, TrafficLogger* pkt_logger, FatTreeTopologyDRB* top);
    ...
    virtual void connect(ConstantErasureCcaSinkDRB& sink, simtime_picosec startTime, uint32_t destination, const Route& routeout, const Route& routein); // for packet switching
    void startflow();
    ...
    bool _drb;
    void set_drb();
    // Per-destination pointer pair for DRB load balancing
    struct DrbPointers {
        uint32_t initial_data_ptr;  // Initial pointer value for data packets
        uint32_t initial_ack_ptr;   // Initial pointer value for ACK packets
        uint32_t data_count;        // Number of data packets sent
        uint32_t ack_count;         // Number of ACK packets sent
        uint32_t data_modulo;       // Modulo value for data packets (num_spines or num_cores)
        uint32_t ack_modulo;        // Modulo value for ACK packets (num_spines or num_cores)
    };
    DrbPointers _drb_state;
    bool _drb_state_initialized;
    std::vector<const Route*> _drb_data_routes;
    std::vector<const Route*> _drb_ack_routes;
    const Route* get_drb_route(uint32_t destination_id, bool is_ack);
    ...
    ConstantErasureCcaSinkDRB* _sink;
...
protected:
    ...
    const Route* _route;
    ConstantErasureCcaPacerDRB _pacer;

    FatTreeTopologyDRB* _top; // Added member for topology
...
class ConstantErasureCcaSinkDRB : public PacketSink, public DataReceiver {
    friend class ConstantErasureCcaSrcDRB;
public:
    ConstantErasureCcaSinkDRB();
...
private:
    // Connectivity
    void connect(ConstantErasureCcaSrcDRB& src, const Route& route);
    const Route* _route;
    string _nodename;
};

...
enum HostLBStrategy {NOLB = 0, SPRAY = 1, PLB = 2, SPRAY_ADAPTIVE = 3, DRB = 4};

EventList eventlist;

// Defined in fat_tree_switch.cpp
...
int main(int argc, char **argv) {
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")){
            ...
            cout << "Enabling BGP-like route propagation (downstream connectivity check)" << endl;
            set_enable_bgp(true);
    ...
    ConstantErasureCcaSrcDRB* sender;
    ConstantErasureCcaSinkDRB* sink;

    const Route* routeout, *routein;
    ...
    list <ConstantErasureCcaSrcDRB*> srcs;
    list <ConstantErasureCcaSinkDRB*> sinks;
    ...
    for (uint32_t c = 0; c < all_conns->size(); c++){
        ...
        sender = new ConstantErasureCcaSrcDRB(eventlist, src, interpacket_delay, NULL, top);  
        ...
        sink = new ConstantErasureCcaSinkDRB();
        ...
        if (route_strategy != SOURCE_ROUTE) {
            if (host_lb == DRB) {
                routeout = sender->get_drb_route(dest, false); // Get forward path (data packets)
                routein = sender->get_drb_route(dest, true);   // Get reverse path (ACK packets)
            } else {
    ...
    while (eventlist.doNextEvent()) {
        if (eventlist.now() > checkpoint) {
            ...
            if (endtime == 0) {
                ...
                bool all_done = true;
                list <ConstantErasureCcaSinkDRB*>::iterator sink_i;
    ...
    list <ConstantErasureCcaSrcDRB*>::iterator src_i;
    ...
    if (get_pkt_delay) { // TODO: condense this
        ...
        for (src_i = srcs.begin(); src_i != srcs.end(); src_i++) {
            ConstantErasureCcaSinkDRB* sink = (*src_i)->_sink;
